...
namespace Helper
{
    public class PageModel
    {
        ...
        public DataTable DataTable
        {
            get
            {
                if (List.GetType() == typeof(DataTable))
                {
                    return List as DataTable;
                } return null;
            }
            private set
            {
        ...
        public static string GetPager(int pageCount, int currentPage, Dictionary<string, string> dic)
        {
            ...
            style.Append(" .pager td:hover  { background-color:#eee;color:#000; border:#06c solid 0px;}");
            style.Append(" .pager .current{ background-color:#fff;color:#000; border:#06c solid 0px; padding:0px 7px; }");
            style.Append("</style>");
        ...
        /// <returns></returns>
        public static string GetHtmlPager(int currentPage, int pageCount, string prefix, string suffix)
        {
            StringBuilder style = new StringBuilder();
            ...
            style.Append(" .pager a { text-decoration:none; color:#039;padding:0 7px;} ");
            style.Append(" .pager a:hover { text-decoration:none;padding:0 7px;} ");
            style.Append(" .pager td  { text-decoration:none; border:#e5e5e5 solid 0px;color:#039; text-align:center; }");
            style.Append(" .pager td:hover  { background-color:#eee;color:#000; border:#06c solid 0px;}");
            style.Append(" .pager .current{ background-color:#fff;color:#000; border:#06c solid 0px; padding:0px 7px; }");
            style.Append("</style>");
            ...
            StringBuilder sb = new StringBuilder(style.ToString());
            sb.Append("\r<table cellpadding=0 cellspacing=1 class=\"pager\">\r<tr>\r");
            sb.Append("<td class=pagerTitle>&nbsp;分页&nbsp;</td>\r");
            sb.Append("<td class=pagerTitle>&nbsp;" + currentPage.ToString() + "/" + pageCount.ToString() + "&nbsp;</td>\r");
            if (currentPage - stepNum < 2)
                pageRoot = 1;
